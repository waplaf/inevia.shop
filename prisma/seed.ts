...
async function main() {
  ...
  const location=await db.location.upsert({where:{storeId_code:{storeId:store.id,code:"ARMAZEM-ONLINE"}},update:{},create:{storeId:store.id,name:"Armazém Inevia.shop",code:"ARMAZEM-ONLINE",type:"WAREHOUSE"}});
  const seededProducts=await db.product.findMany({where:{storeId:store.id}});
  for(const product of seededProducts) await db.inventoryLevel.upsert({where:{locationId_sku:{locationId:location.id,sku:product.sku}},update:{available:product.stock},create:{locationId:location.id,productId:product.id,sku:product.sku,available:product.stock,minimumLevel:5}});
